...
void test_write__success()
{
	...
	VA segment_starting_address = segment->starting_va;
	...
	*write_buffer = (VA)malloc(write_buffer_size);
	...
	int return_code = _write(segment_starting_address, *write_buffer, write_buffer_size);
	assert(return_code == _SUCCESS && memcmp(
            segment_starting_address,
            *write_buffer,
            write_buffer_size
	) == 0);

	*written_adress = segment_starting_address;
}

void test_write__wrong_params()
{
	segment* segment = _segment_table->records[0].segment_ptr;
	VA segment_starting_address = *_vas;
	...
	*write_buffer = (VA)malloc(write_buffer_size);
	...
	int return_code = _write(segment_starting_address - 1, *write_buffer, write_buffer_size);
	assert(return_code == _WRONG_PARAMS);
...
void test_read__success() 
{
	...
	*read_buffer = (VA)malloc(read_buffer_size);
	...
	int return_code = _read(segment_starting_address, *read_buffer, read_buffer_size);
	assert(return_code == _SUCCESS && memcmp(
            segment_starting_address,
            *read_buffer,
            read_buffer_size
	...
	*read_adress = segment_starting_address;
}
...
void test_read__wrong_params() 
{
	...
	*read_buffer = (VA)malloc(read_buffer_size);
	...
	int return_code = _read(segment_starting_address - 1, *read_buffer, read_buffer_size);
	assert(return_code == _WRONG_PARAMS);
...
void print_malloc_results()
{
	...
	if (*malloc_ptr != NULL)
	{
		printf("Allocated memory size was %zu, address (VA) was %p", malloc_size, *malloc_ptr);
	}
	else
	{
		printf("Allocated memory size may be %zu, but there is no enough free space", malloc_size);
	}
}
...
void print_free_results()
{
	...
	if (*freed_ptr != NULL)
	{
		printf("Freed memory size was %zu, address (VA) was %p", freed_size, *freed_ptr);
	}
	else
	{
		printf("Freed memory size may be %zu, but there is no such address (VA) %p", freed_size, *freed_ptr);
	}
}
...
void print_write_results() 
{
	...
	if (*written_adress != NULL)
	{
		*(*write_buffer + write_buffer_size) = '\0';
		printf("Written buffer size was %zu, address (VA) was %p, buffer was %s",
			write_buffer_size,
			*written_adress, 
			*write_buffer);
	}
	...
	{
		printf("Written memory size may be %d, but is are no such address or segmentation fault occured", write_buffer_size);
	}
}
...
void print_read_results() 
{
	...
	if (*read_adress != NULL)
	{
		*(*read_buffer + read_buffer_size) = '\0';
		printf("Read buffer size was %zu, address (VA) was %p, buffer is %s",
			read_buffer_size,
			*read_adress,
			*read_buffer);
	}
	...
	{
		printf("Read memory size may be %zu, but is are no such address or segmentation fault occurred", read_buffer_size);
	}
}
