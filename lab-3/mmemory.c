...
int _malloc (VA* ptr, size_t szBlock)
{
	...
	return_code = _init_address(_first_free_va, szBlock);
	if (return_code != _SUCCESS) 
	{
		*ptr = NULL;
		return return_code;
	...
	*ptr = *_first_free_va;
	...
	else _first_free_va = _first_null_content_address(_vas, _vas, _last_free_va);
...
int _free (VA ptr)
{
	...
	segment* found_segment = _find_segment_by_starting_address(ptr);
	if (found_segment == NULL) return _WRONG_PARAMS;

	_clear_space_region(
            _pas + _address_abs_offset(_pas, found_segment->starting_pa),
		found_segment->size
		);
	_clear_space_region(
            _vas + _address_abs_offset(_vas, found_segment->starting_va),
		found_segment->size
		);
