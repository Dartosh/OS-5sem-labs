...
segment* _find_segment_by_starting_address (VA segment_starting_va)
{
	for (uint record_index = 0; record_index < _segment_table->current_records_count; record_index++)
	{
...
int _find_segment_by_inner_address (VA inner_adress, size_t segment_region_size, segment** found_segment)
{
	VA seg_starting_address = NULL;
	size_t segment_region_size_copy = segment_region_size;
	uint segment_address_offset = 0;
	for (uint record_index = 0; record_index < _segment_table->current_records_count; record_index++)
	{
        ...
        seg_starting_address = _segment_table->records[record_index].segment_ptr->starting_va;
		if (seg_starting_address == inner_adress)
		{
		...
		{
            segment_address_offset = 0;
			while (segment_address_offset < segment_region_size_copy)
			{
				if (seg_starting_address + segment_address_offset == inner_adress)
				{
