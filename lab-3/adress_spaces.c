...
int	_init_address(VA* address, size_t content_size)
{
	*address = (VA)malloc(content_size);
	return (*address == NULL) ? _UNKNOWN_ERR : _SUCCESS;
}

uint _address_abs_offset(VA* space, VA adress)
{
	for (uint address_offset = 0; address_offset < _vas_size; address_offset++)
	{
		if (_vas[address_offset] == adress) return address_offset;
	}

	return _FORBIDDEN_ADRESS_OFFSET;
}

void _clear_space_region(VA* starting_address, size_t size)
{
	if (starting_address == NULL || *starting_address == NULL) return;

	uint curr_address_offset = 0;
	while (curr_address_offset < sizeof(**(starting_address)) * size)
	{
		*(starting_address + curr_address_offset) = NULL;
		curr_address_offset++;
	}
}

VA* _first_null_content_address(VA* space, VA* starting_address, VA* last_free_space_adress)
{
	if (starting_address == NULL) starting_address = space;
	VA* starting_address_copy = starting_address;

	while (starting_address_copy <= last_free_space_adress)
	{
		if (*starting_address_copy == NULL) return starting_address_copy;
		starting_address_copy++;
	}
...
{
	uint address_offset = _address_abs_offset(_vas, va);
	if (address_offset == _FORBIDDEN_ADRESS_OFFSET) return _WRONG_PARAMS;

	segment* owner = (segment*)malloc(sizeof(segment));

	int return_code = 0;
	return_code = _find_segment_by_inner_address(va, region_size, &owner);
	...
	*pa = owner->starting_pa + address_offset;
	return _SUCCESS;
}

VA* _request_free_space_region(VA* space, VA* last_free_space_address, size_t size)
{
	if (size == 0) return space;

	VA* starting_address = space;
	size_t nulled_space_size = 0;
	while (*starting_address != NULL)
	{
		nulled_space_size = _nulled_space_region_size(space, starting_address);
		if (nulled_space_size >= size) return starting_address;

		starting_address++;
        starting_address = _first_null_content_address(
                space,
                starting_address,
                last_free_space_address
        );
	}
...
int	_organize_space_for_segment_allocation(
	VA* space,
	VA** first_free_space_address,
	VA* last_free_space_address,
	size_t segment_size
)
{
#define SIZE segment_size
#define FIRST first_free_space_address
#define LAST last_free_space_address
	...
	if (*FIRST + SIZE > LAST)
	{
		...
		*first_free_space_address = (VA *) *free_space;
...
int _load_segment (segment* segment)
{
	...
	return_code = _init_address(_first_free_pa, segment->size);
	if (return_code != _SUCCESS) return return_code;
	...
	else _first_free_pa = _first_null_content_address(_pas, _pas, _last_free_pa);
...
int _load_adjacent_segments (segment* central_segment)
{
	...
	{
		VA* left_adj_seg = _vas + _address_abs_offset(
                _vas,
		...
		if (left_adj_seg + 1 != _vas)
		{
			...
			int return_code = _find_segment_by_inner_address(*left_adj_seg, 0, &left_seg);
			if (return_code != _SUCCESS) return return_code;
	...
	{
		VA* right_adj_seg = _vas + _address_abs_offset(
                _vas,
		...
		if (right_adj_seg - 1 != _last_free_va)
		{
			...
			int return_code	= _find_segment_by_inner_address(*right_adj_seg, 0, &right_seg);
			if (return_code != _SUCCESS) return return_code;
...
void _print_space (VA* space, size_t address_offset_limit, const char* space_name)
{
	printf("\n -------------------------------\n");
	printf("| ");
	printf("%s", space_name);
	...
	for (uint offset = 0; offset < address_offset_limit; offset++)
	{
